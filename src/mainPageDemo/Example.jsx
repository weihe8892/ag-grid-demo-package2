...
const Example = ({onOpen}) => {
   ...
   useEffect(() => {
      ...
      setGridTheme('ag-theme-theme-light');
   }, []);
   ...
   const createRowItem = (row, colCount) => {
      ...
      rowItem.language = tempId / (row * 15); //countryData.language;

      const firstName = firstNames[row % firstNames.length];
      const lastName = lastNames[row % lastNames.length];
      rowItem.name = tempId / row; //firstName + ' ' + lastName;
   ...
   useEffect(() => {
      ...
      defaultCols = [...defaultCols, ...monthGroup.children];

      defaultColCount = defaultCols.length; //22;
      // }

      defaultCols.forEach(c => {
         c.filter = 'agTextColumnFilter';
         c.filterValueGetter = `data[colDef.field]`;
         // c.valueFormatter = `data[colDef.field]`;
         c.tooltipField = 'name';
         c.sort = 'asc';
         c.sortIndex = 0;
      });
      ...
      const newRowsCols = [
         ...
         [1200, defaultColCount],
         [10000, defaultColCount],
      ];
   ...
   const createCols = () => {
      ...
      for (let col = defaultColCount; col < colCount; col++) {
         ...
         const colDef = {headerName: colName, field: 'col' + col, width: 100, editable: true};
         columns.push(colDef);
      }
